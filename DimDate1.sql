...
CREATE TABLE	[dbo].[DimDate]
	(	[DateKey] INT primary key, 
		...
		[IsHolidayEC] BIT Null, --Flag 1=National Holiday, 0-No National Holiday
		[HolidayEC] VARCHAR(50)
	)
GO
...
--New Years Day
	...
	UPDATE [dbo].[DimDate]
		...
		/*Update Ecuador Holidays */
	
-- Víspera de Año Nuevo 
	UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Víspera de Año Nuevo'
	WHERE [Month] = 12 AND [DayOfMonth]  = 31

-- Año nuevo 
	UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Año Nuevo'
	WHERE [Month] = 1 AND [DayOfMonth]  = 1

-- Navidad 
   UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Navidad'
	WHERE [Month] = 12 AND [DayOfMonth]  = 25

-- Spring Nochebuena 
	UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Nochebuena'
	WHERE [Month] = 12 AND [DayOfMonth]  = 24

	UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Día del Trabajo'
	WHERE [Month] = 5 AND [DayOfMonth]  = 1

	UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Batalla de Pichincha'
	WHERE [Month] = 5 AND [DayOfMonth]  = 24
	UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Primer Grito de la Independencia'
	WHERE [Month] = 8 AND [DayOfMonth]  = 10
	UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Independencia de cuenca'
	WHERE [Month] = 11 AND [DayOfMonth]  = 3
	UPDATE [dbo].[DimDate]
		SET HolidayEC = 'Independencia de Guayaquil'
	WHERE [Month] = 10 AND [DayOfMonth]  = 9

	UPDATE [dbo].[DimDate]
		SET IsHolidayEC = CASE WHEN HolidayEC  IS NULL THEN 0 WHEN HolidayEC  IS NOT NULL THEN 1 END
/*******************************************************************************************************************************************************/
...
CodigoFactVentas varchar(30)  not null,
DateKey int,
CodigoPuntoVenta varchar(20) not null,
CodigoCliente varchar(20) not null,
CodigoProducto varchar(20) not null,
Cantidad int,
CostoTotalVenta money,
CostoProduccion float

)
Go

AlTER TABLE FactVentas ADD CONSTRAINT FK_CodigoPuntoVenta FOREIGN KEY (CodigoPuntoVenta)REFERENCES DimPuntodeVenta(CodigoPuntoVenta);
AlTER TABLE FactVentas ADD CONSTRAINT FK_CodigoCliente FOREIGN KEY (CodigoCliente)REFERENCES DimCliente(CodigoCliente);
AlTER TABLE FactVentas ADD CONSTRAINT FK_CodigoProducto FOREIGN KEY (CodigoProducto)REFERENCES DimProducto(CodigoProducto);

Go

AlTER TABLE FactVentas ADD CONSTRAINT FK_VentasDateKey FOREIGN KEY (DateKey)REFERENCES DimDate(DateKey);
Go
